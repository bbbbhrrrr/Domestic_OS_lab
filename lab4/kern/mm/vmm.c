...
do_pgfault(struct mm_struct *mm, uint_t error_code, uintptr_t addr) {
    ...
    *   PTE_W                              // page table/directory entry flags bit : Writeable
    *   PTE_U                              // page table/directory entry flags bit : User can access
    * VARIABLES:
    ...
    */
    ...
    //(2) if this pte is not present, then alloc a page & map pa<--->la with linear address la
    if (!(*ptep & PTE_V)) {
        ...
        cprintf("do_pgfault failed: ptep %x is already present\n", *ptep);
        goto failed;
    }
   ret = 0;
failed:
