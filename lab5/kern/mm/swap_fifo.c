...
_fifo_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
    ...
    /* Select the victim */
    list_entry_t *victim = head->prev; // 选择最早进入的页面
    assert(victim != head); // 确保链表不为空

    struct Page *page = le2page(victim, pra_page_link); // 获取页面地址
        list_del(victim); // 将该元素从链表中移除
    assert(page != NULL);
    *ptr_page = page; // 设置页面地址到 ptr_page
