...
#define BEING_CHECK_VALID_VADDR 0X2000
#define CHECK_VALID_VADDR (CHECK_VALID_VIR_PAGE_NUM+1)*0x2000
...
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)
{
 	...
 	// (2) 利用 addr 对应的 pte 将内容从 swapfs 中读入 
 	pte_t *ptep = get_pte(mm->pgdir, addr, 0); 
 	int r;

	if ((r = swapfs_read((*ptep), new_page)) != 0)
	{
		assert(r!=0);
	}
	*ptr_result=new_page; 
...
check_content_set(void)
{
     ...
     *(unsigned char *)0x2010 = 0x0a;
     assert(pgfault_num==1);
     *(unsigned char *)0x4000 = 0x0b;
     assert(pgfault_num==2);
     *(unsigned char *)0x4010 = 0x0b;
     assert(pgfault_num==2);
     *(unsigned char *)0x6000 = 0x0c;
     assert(pgfault_num==3);
     *(unsigned char *)0x6010 = 0x0c;
     assert(pgfault_num==3);
     *(unsigned char *)0x8000 = 0x0d;
     assert(pgfault_num==4);
     *(unsigned char *)0x8010 = 0x0d;
     assert(pgfault_num==4);
}
...
check_swap(void)
{
    //backup mem env
     ...
     //setup the temp Page Table vaddr 0~8MB
     cprintf("setup Page Table for vaddr 0X2000, so alloc a page\n");
     pte_t *temp_ptep=NULL;
     temp_ptep = get_pte(mm->pgdir, BEING_CHECK_VALID_VADDR, 1);
     assert(temp_ptep!= NULL);
     cprintf("setup Page Table vaddr 0~8MB OVER!\n");
    ...
    // assert(alloc_page() == NULL);
     ...
     for (i= 0;i<CHECK_VALID_PHY_PAGE_NUM;i++) {
         check_ptep[i]=0;
         check_ptep[i] = get_pte(pgdir, (i+1)*0x2000, 0);
         //cprintf("i %d, check_ptep addr %x, value %x\n", (i+1)*0x2000, check_ptep[i], *check_ptep[i]);
